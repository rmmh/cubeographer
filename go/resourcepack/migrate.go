...
func DataVersionFromPackFormat(format int) int {
	switch format {
	case 4: // 1.13 - 1.14.4
		return 1500
	case 5: // 1.15 - 1.15.2
	...
	case 9: // 1.19
		return 3100
	case 11, 12, 13, 14, 15: // 1.19.3 - 1.20.1
		return 3300
	case 18, 22: // 1.20.2 - 1.20.4
		return 3500
	case 32, 34: // 1.20.5 - 1.21.1
		return 3800
	default:
		if format > 34 {
			return 4000
...
func MigrateName(name string, renameMap map[string]string) string {
	hasPrefix := strings.Contains(name, ":")
	prefix := ""
	cleanName := name
	if hasPrefix {
	...
	subPrefix := ""
	if strings.HasPrefix(cleanName, "block/") {
		subPrefix = "block/"
		cleanName = cleanName[len("block/"):]
	} else if strings.HasPrefix(cleanName, "item/") {
		subPrefix = "item/"
		cleanName = cleanName[len("item/"):]
	}

	if newName, ok := renameMap[cleanName]; ok {
		return prefix + subPrefix + newName
	}
	return name
}

func MigrateModel(model *Model, renameMap map[string]string) {
	if model == nil {
		return
	}
	if model.Parent != "" {
		model.Parent = MigrateName(model.Parent, renameMap)
	}
	if model.Textures != nil {
		for k, v := range model.Textures {
			model.Textures[k] = MigrateName(v, renameMap)
		}
	}
	for i := range model.Overrides {
		model.Overrides[i].Model = MigrateName(model.Overrides[i].Model, renameMap)
	}
}

func MigrateBlockState(bs *BlockState, renameMap map[string]string) {
	if bs == nil {
		return
	}
	if bs.Variants != nil {
		for k, specs := range bs.Variants {
			for i := range specs {
				specs[i].Model = MigrateName(specs[i].Model, renameMap)
			}
			bs.Variants[k] = specs
		}
	}
	for i := range bs.Multipart {
		specs := bs.Multipart[i].Apply
		for j := range specs {
			specs[j].Model = MigrateName(specs[j].Model, renameMap)
		}
		bs.Multipart[i].Apply = specs
	}
}
